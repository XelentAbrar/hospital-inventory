  ...
  <div class="a4-size relative mx-auto flex flex-col rounded-lg shadow-md">
    <div class="bg-white overflow-hidden a4-size relative mx-auto shadow-sm rounded-lg">
      <div style="font-family: Arial, Helvetica, sans-serif; padding: 20px;display: flex;flex-direction: column;">
          ...
          <div style="display: flex;align-items: center;justify-content: space-between;flex: initial;width: 100%;margin: 5px auto;">
            <div style="width: 50%;">
                <div style="display: flex;align-items: center;">
                    ...
                    <p style="margin: 0;margin-left: 2px;text-align: left;font-size: 14px; ">{{ materialReturnNote.mrn_no }}</p>
                </div>
            </div>
            <div style="width: 50%;display: flex;justify-content: start;">
                <div style="display: flex;align-items: center;">
                    ...
                    <h4 style="margin:0 10px;font-weight:bold; width: 10px;text-align: center;">:</h4>
                    <p style="margin: 0;margin-left: 2px;text-align: left;white-space: nowrap;font-size: 14px; ">   {{ new Date(materialReturnNote.date).toLocaleDateString('en-GB').replace(/\//g, '-') }}
                    </p>
                </div>
            </div>
          </div>
          <div style="display: flex;align-items: center;justify-content: space-between;flex: initial;width: 100%;margin: 5px auto;">
            <!-- <div style="width: 50%;display: flex;justify-content: start;">
                <div style="display: flex;align-items: center;">
                    ...
                    <h4 style="margin:0 10px;font-weight:bold; width: 10px;text-align: center;">:</h4>
                    <p style="margin: 0;margin-left: 2px;text-align: left;font-weight: 500;font-size: 14px; ">{{ materialReturnNote?.department?.name }}</p>
                </div>
            </div> -->
        ...
        <div>
          <p style="font-size: 20px;font-weight: 600;text-align: center;margin: 10px 0;">MATERIAL RETURN ISSUE DETAIL</p>
        </div>
        <div class="ring-1 ring-gray-300 sm:rounded-lg overflow-x-auto overflow-y-hidden">
          <table class="min-w-full border-separate border-spacing-0 divide-y divide-gray-300 overflow-auto">
            ...
            <tbody>
              <tr v-if="materialReturnNote.detail && materialReturnNote.detail.length" class="odd:bg-gray-50 even:bg-white divide-x divide-gray-300" v-for="(material, index) in materialReturnNote.detail" :key="material.id">
            ...
            <tfoot>
                <tr>
                    ...
                    <td class="whitespace-nowrap relative text-center border-b border-gray-300 bg-gray-300 font-bold py-1">  {{ sum(materialReturnNote, 'qty', 'material') }}</td>
                </tr>
            </tfoot>
          </table>
        </div>
...
const sum = (materialReturnNote, val) => {
  let total = 0;

  if (materialReturnNote && Array.isArray(materialReturnNote.detail) && materialReturnNote.detail.length > 0) {
    materialReturnNote.detail.forEach(res => {
