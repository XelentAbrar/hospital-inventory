    ...
    <div class="bg-white overflow-hidden a4-size relative mx-auto shadow-sm rounded-lg">
      <div
              style="font-family: Arial, Helvetica, sans-serif; padding: 10px 20px;display: flex;flex-direction: column;">
                ...
                <div
                    ...
                    <div style="width: 50%;">
                        <div style="display: flex;align-items: center;">
                            <h4 style="margin: 0;font-family: sans-serif; width: 100px;font-weight:600;font-size: 14px;">Demand #</h4>
                            <h4 style="margin:0 10px;font-weight:bold; width: 10px;text-align: center;">:</h4>
                            <p style="margin: 0;margin-left: 2px;text-align: left;font-size: 14px;">{{ demands.demand_no }}</p>
                        </div>
                    </div>
                    <div style="width: 50%;display: flex;justify-content: start;">
                        <div style="display: flex;align-items: center;">
                            ...
                            <p style="margin: 0;margin-left: 2px;text-align: left;white-space: nowrap;font-size: 14px;">
                                {{ new Date(demands.date).toLocaleDateString('en-GB').replace(/\//g, '-') }}
                ...
                <div   style="display: flex;align-items: center;justify-content: space-between;flex: initial;width: 100%;margin: 5px auto;">
                    <div style="width: 50%;display: flex;justify-content: start;">
                        <div style="display: flex;align-items: center;">
                            ...
                            <h4 style="margin:0 10px;font-weight:bold; width: 10px;text-align: center;">:</h4>
                            <p style="margin: 0;margin-left: 2px;text-align: left;font-weight: 500;font-size: 14px;">{{ demands?.department?.name }}</p>
                        </div>
                    </div>
                    <div style="width: 50%;display: flex;justify-content: start;">
                        <div style="display: flex;align-items: center;">
                ...
                <div>
                    <p
                        style="font-size: 22px;font-weight: 600;text-align: center;margin: 0;margin-bottom: 10px;margin-top: 10px;">Demand Detail</p>
                </div>
              <div class="ring-1 ring-gray-300 sm:rounded-lg overflow-x-auto overflow-y-hidden">
                    ...
                    <tbody>
                        <tr v-if="demands.detail && demands.detail.length" class="odd:bg-gray-50 even:bg-white divide-x divide-gray-300" v-for="(dem, index) in demands.detail" :key="dem.id">
                            <td class="whitespace-nowrap relative text-center border-b border-gray-300">{{index + 1 }}</td>
                            <td class="whitespace-nowrap relative text-center border-b border-gray-300">{{ dem?.product?.name }}</td>
                            <td class="whitespace-nowrap relative text-center border-b border-gray-300">{{ dem?.qty }}</td>
                        </tr>
                        <tr v-else>
                    ...
                    <tfoot>
                        <tr>
                            ...
                            <td class="whitespace-nowrap relative text-center border-b border-gray-300 bg-gray-300 font-bold py-1">  {{ formatNumber(sum(demands, 'qty', 'demand')) }}</td>
                        </tr>
                    </tfoot>
                  </table>
                </div>
...
const sum = (demands, val) => {
  let total = 0;

  if (demands && Array.isArray(demands.detail) && demands.detail.length > 0) {
    demands.detail.forEach(res => {
